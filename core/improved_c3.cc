...
vector<VectorXd> ImprovedC3::SolveQP(const VectorXd &x0,
  ...
  for (int i = 0; i < N_; ++i)
  {
    prog_.AddBoundingBoxConstraint(0,std::numeric_limits<double>::infinity(),u_.at(i)[2]);
    prog_.AddBoundingBoxConstraint(0,std::numeric_limits<double>::infinity(),u_.at(i)[3]);
  }
