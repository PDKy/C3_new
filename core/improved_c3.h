...
public:
   ...
   */
  ImprovedC3(const LCS &LCS, const CostMatrices &costs,
             const std::vector<Eigen::VectorXd> &x_desired,
             const C3Options &options);

  virtual ~ImprovedC3() = default;
   ...
   * total). xᵢᵀQᵢxᵢ − 2x[d]ᵢQᵢᵀxᵢ xᵢ     : State variable at the ith timestep
  ...
  std::vector<Eigen::VectorXd> GetSDFSolution() { return *gamma_sol_; }
...
protected:
  ...
  std::vector<std::vector<Eigen::VectorXd>> warm_start_gamma_;
  bool warm_start_;
  const int N_;
  const int n_x_;      // n
   ...
   */
  Eigen::VectorXd
  SolveSingleProjection(const Eigen::MatrixXd &U,
...
private:
  ...
  SolveProjection(const std::vector<Eigen::MatrixXd> &U,
                  std::vector<Eigen::VectorXd> &WZ, int admm_iteration);
  ...
  std::unique_ptr<std::vector<Eigen::VectorXd>> gamma_sol_;
